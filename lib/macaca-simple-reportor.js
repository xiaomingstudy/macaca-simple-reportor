...
function Reporter(runner) {
    ...
    runner.on('suite end', function(suite) {
        ...
        if (indents === 1) {
            ...
            fs.writeFileSync(resultLog, ThisResultLog + '],\n');
            if (RESULT_STATUS == 1) {
                fs.appendFileSync(resultLog, '\"status\":\"passed\",' + '\"Case_error_message\":\"' + errorInfo + '\"}');
            } else {
                fs.appendFileSync(resultLog, '\"status\":\"failed\",' + '\"Case_error_message\":\"' + errorInfo + '\"}');
            }
        }
    });
    ...
    runner.on('pass', function(test) {
        ...
        fs.appendFileSync(resultLog, `{${STEP_NAME}"${test.title}"${SEP_TOKEN}${STEP_RESULT}true${SEP_TOKEN}${SHOT_NAME}"${stepCountShot}"${SEP_TOKEN}${ERROR_MESSAGE}"null"},${EOL}`);
    });
    ...
    runner.on('fail', function(test, err) {
        ...
        errorInfo = `${err.message.replace(/(?:\r\n|\r|\n|\")/g, ' ')}`;
        fs.appendFileSync(resultLog, `{${STEP_NAME}"${test.title}"${SEP_TOKEN}${STEP_RESULT}false${SEP_TOKEN}${SHOT_NAME}"${stepCountShot}"${SEP_TOKEN}${ERROR_MESSAGE}"${err.message.replace(/(?:\r\n|\r|\n)/g, ' ')}"},${EOL}`);
    });
